...
export default function ContentHUb({ locale }) {
  ...
  const meta = staticMetaByRoute["/content-gallery"];
  ...
  const handleOpenModal = (content) => {
    let embedUrl = content.videoUrl.replace(
      "youtu.be/",
  ...
  const videos = [
    {
      name: "Mr. Chari",
      description:
        "Rising from financial hardships to owning multiple tractors",
            ...
            <div className="flex sm:gap-4 gap-2 mb-4 mt-2">
                ...
                >
                  {activeTab === "videoData" ? (
                    <div className="text-base">
                      {" "}
                      <Btn text={t("Home.Videos")} bgColor={true} />
                    </div>
                  ) : (
                    <div className="text-base">
                      {" "}
                      <Btn text={t("Home.Videos")} bgColor={false} />
                    </div>
                  )}
                </Tab>
              </div>
              <div className="sm:w-1/6 w-1/2">
                ...
                >
                  {activeTab === "blogData" ? (
                    <div className="text-base">
                      <Btn text={t("Home.Blog")} bgColor={true} />
                    </div>
                  ) : (
                    <div className="text-base">
                      {" "}
                      <Btn text={t("Home.Blog")} bgColor={false} />
                    </div>
                  )}
                </Tab>
              </div>
                ...
                {videos.map((video, index) => (
                  <div className="border p-1 relative" key={index}>
                    <div className="gap-3 sm:items-center items-start">
                      ...
                      <div>
                        <div className="sm:text-base text-sm absolute right-3">
                          {video.date}
                        </div>
                        <div className="font-semibold py-2 sm:text-base text-[14px]">
                          {video.name}
                        </div>
            ...
            content={
              modalContent?.videoUrl ? (
                <iframe
                  width="100%"
                  height="315"
                  src={modalContent.videoUrl.replace(
                    "youtube.com",
